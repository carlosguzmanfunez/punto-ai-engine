...
version: 0.2.0
role: BUILDER
description: Procedimiento compacto para convertir una verificación fallida en una reparación discriminante, con la ampliación y el cambio en la misma propuesta.
---
...
6. **EXPLAIN OR ADDRESS**: para cada recurso relevante, o lo cambias, o lo declaras en
   ...
   está fuera del alcance. Si ya conoces el cambio mínimo del recurso que está fuera de alcance, pide
   la `scope_expansion` **e incluye ese cambio en la misma respuesta**: PUNTO evalúa la ampliación
   antes de validar los cambios y decide si la autoriza; si no la autoriza, no se aplica nada. Que
   una verificación lea un fichero no obliga a tocarlo.
