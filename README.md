...
### ENGINE-6.0.3 — Continuidad de ejecución y presupuesto

Tercera pasada, sobre las tres fronteras residuales que encontró la auditoría de 6.0.2:

| Hallazgo | Cómo se cierra |
| --- | --- |
| V603-01 Saldo no aplicado | El saldo de modelo y de tokens es una **cota**: en el Architect y el Planner entra en sus `limits` como `min(máximo del rol, saldo restante)` —incluidos los intentos de reparación—, y en los demás roles, cuyo contrato no admite una cota por ejecución, el adaptador **no invoca** al runner si su máximo declarado no cabe en el saldo (`Camus.declared_model_limits`). Con una sola llamada disponible, el Architect llama al proveedor una vez, no cinco |
| V603-02 Reserva no durable | Cada intento reserva, **consume y persiste el checkpoint antes** de invocar al rol: una llamada iniciada cuenta aunque el proceso muera antes de recibir la respuesta, y la reanudación no vuelve a gastarla. Conservador antes que doble gasto |
| V603-03 Rol no satisfecho | Una etapa solo avanza cuando sus roles obligatorios tienen un paso `COMPLETED` **sin hallazgos bloqueantes** (o `NOT_APPLICABLE`). Un `PROVIDER_UNAVAILABLE`, `BLOCKED`, `NEEDS_REPAIR` o `FAILED` deja el rol pendiente: al reanudar se reabre el mismo rol, y ninguna etapa posterior corre sin la anterior satisfecha. Un efecto `UNKNOWN` sigue bloqueando para reconciliación sin repetir al Developer |
| V603-04 Handoff parcial | `workflow/handoff.py` publica y resuelve **toda** la pipeline: resultado del Developer, informes de QA, Security, Reviewer, CrossAudit y VisualQA, y la evidencia visual (`VISUAL_EVIDENCE`). Cada adaptador reconstruye su entrada desde el checkpoint y los almacenes, sin closures; si falta un artefacto se bloquea con `WORKFLOW_INCOMPLETE_EVIDENCE` en vez de inventarlo. La evidencia previa al workflow (sesión web) viaja como `WorkflowRequest.evidence_references` |
...
- El sistema de ficheros del proveedor de tareas (la API de tareas de fases 0/1) conserva su booleano
  `approved=`: no conduce el kernel autónomo, cuyo `resume` exige `HumanApprovalProof`.
