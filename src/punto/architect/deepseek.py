...
class DeepSeekArchitectRunner(ArchitectRunner):
            ...
            for attempt in range(1, self._limits.max_attempts + 1):
                ...
                if model_calls >= self._limits.max_model_calls:
                    raise PlanningLimitExceededError(
                        BLOCKED_ARCHITECT_CALLS, model_calls, self._limits.max_model_calls
                    )
    ...
    def _assert_token_budget(self, usage: ModelUsage) -> None:
        ...
        if usage.prompt_tokens > self._limits.max_input_tokens:
            raise PlanningLimitExceededError(
                BLOCKED_ARCHITECT_TOKENS, usage.prompt_tokens, self._limits.max_input_tokens
            )
        if usage.completion_tokens > self._limits.max_output_tokens:
            raise PlanningLimitExceededError(
                BLOCKED_ARCHITECT_TOKENS, usage.completion_tokens, self._limits.max_output_tokens
            )
    ...
    # --------------------------------------------------------------- auditoría
    def _audit_request_started(self, request: ArchitectRequest) -> None:
